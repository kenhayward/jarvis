...
def _write_mcp_config(home: Path) -> Path:
    ...
    # What this machine cannot do, told to the MCP child so it can leave
    # those tools out of `tools/list` entirely. It runs as a separate
    # process and imports nothing of JARVIS's, so the env block is the
    # channel that already exists — the same one carrying the endpoint and
    # the token path. Empty on a complete host, and omitted rather than set
    # empty so the common case adds nothing to the file.
    withdrawn = sorted(jarvis_platform.withdrawn_tools())
    if withdrawn:
        child_env_block["JARVIS_DISABLED_TOOLS"] = ",".join(withdrawn)
        log.info("platform %s cannot offer: %s",
                 jarvis_platform.current().name, ", ".join(withdrawn))
...
async def internal_tool(request: Request):
    ...
    # tools/list — so reaching here means a brain older than the current
    # capability set, or something calling `/internal/tool` directly. Same
    # answer either way, because from the caller's side the tool genuinely
    # does not exist here; the log line is what tells the difference.
    if not jarvis_platform.allows_tool(tool):
        log.warning("refused %s: %s cannot do that", tool,
                    jarvis_platform.current().name)
        return _tool_reply(False, f"Unknown tool: {tool}")
